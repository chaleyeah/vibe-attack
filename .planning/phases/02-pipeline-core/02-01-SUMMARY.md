...
tags: [deps, config, tests, wave0]
requires: [phase-01-foundation]
provides: [vad-config, stt-config, wake-config, wave0-tests]
affects: [Cargo.toml, src/config.rs, tests/]
tech_stack:
  added:
    - silero-vad-rust
    - sherpa-onnx
    ...
    - serde_json
  patterns:
    - strict config parsing (deny_unknown_fields)
    - env-gated ignored integration tests
key_files:
  created:
  ...
  modified:
    ...
    - config.example.yaml
decisions:
  - "Pin `ort` to `2.0.0-rc.10` to keep `silero-vad-rust` compiling (upstream RC API break observed)."
  - "`whisper-rs` is an opt-in dependency via `--features stt` so default `cargo test -q` stays green without native build tooling."
metrics:
