...
plan: 03
subsystem: pipeline
tags: [whisper-rs, whisper.cpp, sherpa-onnx, silero-vad, onnxruntime, jsonl, threads]

requires:
  - phase: 02-pipeline-core
    provides: "VAD segmentation + bounded drop-oldest queue helper from 02-02"
provides:
...
affects: [phase-03-dispatch, output-contract, latency-budget]

tech-stack:
  added: []
  patterns:
    - "Tokio only for signals; all ML inference on dedicated std::thread workers"
    - "Single stdout writer thread; all diagnostics via tracing to stderr"
    - "Bounded channels with drop-oldest try_send policy at overload boundaries"
